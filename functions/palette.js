...
function buildGradientStops(accent) {
  const lightColors = [
    hslToHex({
      h: accent.h,
      s: adjustSaturation(accent.s, 0.4, 0.08),
      l: adjustLightness(accent.l, 0.42, 0.52),
    }),
    hslToHex({
      h: accent.h,
      s: adjustSaturation(accent.s, 0.52, 0.05),
      l: adjustLightness(accent.l, 0.26, 0.62),
    }),
    hslToHex({
      h: accent.h,
      s: adjustSaturation(accent.s, 0.65),
      l: adjustLightness(accent.l, 0.12, 0.72),
    }),
  ];
  ...
  const darkColors = [
    hslToHex({
      h: accent.h,
      s: adjustSaturation(accent.s, 0.55, 0.04),
      l: adjustLightness(accent.l, 0.14, 0.38),
    }),
    hslToHex({
      h: accent.h,
      s: adjustSaturation(accent.s, 0.62, 0.02),
      l: adjustLightness(accent.l, 0.04, 0.3),
    }),
    hslToHex({
      h: accent.h,
      s: adjustSaturation(accent.s, 0.72),
      l: adjustLightness(accent.l, -0.04, 0.22),
    }),
  ];
...
export async function onRequest({ request }) {
  ...
  const imageParam =
    url.searchParams.get("image") || url.searchParams.get("url");
  if (!imageParam) {
  ...
  // Local environment: don't use CF-specific cache/image transform. Just fetch.
  let upstream;
  try {
  ...
  try {
    ...
    console.error("Failed to read upstream body", err);
    return new Response(
      JSON.stringify({ error: "Failed to read image body" }),
      {
        status: 502,
        headers: createJsonHeaders(502),
      }
